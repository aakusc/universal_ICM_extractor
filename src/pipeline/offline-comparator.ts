/**
 * Offline Comparator — Compare builder config against extracted CIQ data (no live API).
 *
 * Reuses PlanTestResult so renderPlanTestReport() works unchanged.
 */
...
// ── Helpers (copied from plan-tester.ts) ──────────────────
...
// ── Types for extraction data shape ───────────────────────
...
export interface ExtractionWorksheet {
  id: string;
  name: string;
  columns: Array<{ name: string; type: string; isDerived?: boolean }>;
}

export interface ExtractionWorkbook {
  id: string;
  name: string;
  worksheets: ExtractionWorksheet[];
}

export interface ExtractionAttributeWorksheet {
  id: string;
  name: string;
  description?: string | null;
  pkType?: string | null;
...
export interface ExtractionData {
  ...
  workbooks: ExtractionWorkbook[];
  attributeWorksheets: ExtractionAttributeWorksheet[];
  employeeAssumptions: {
    columns: string[];
    recordCount?: number;
...
const statusScores: Record<string, number> = {
  'match': 1.0, 'partial': 0.5, 'mismatch': 0.0,
  'missing-in-ciq': 0.0, 'extra-in-ciq': 0.5, 'not-applicable': -1,
};
...
export function compareOffline(
  ...
  // Filter extraction to selected plans
  const selectedPlans = extraction.plans.filter(p => selectedPlanIds.includes(p.id));
  // Flatten all worksheets from all workbooks
  const allWorksheets = extraction.workbooks.flatMap(wb =>
    wb.worksheets.map(ws => ({ ...ws, workbookName: wb.name }))
  );
  ...
  const matchedPlan = selectedPlans.find(p => fuzzyMatch(config.planStructure.planName, p.name));
  ...
  items.push({
    ...
    details: matchedPlan
      ? `Found plan "${matchedPlan.name}"`
      : `No plan matching "${config.planStructure.planName}" in selected plans. Available: ${selectedPlans.map(p => p.name).join(', ') || 'none'}`,
  });

  if (matchedPlan) {
    const plannedPeriod = config.planStructure.periodType.toUpperCase();
    const actualPeriod = matchedPlan.periodType?.toUpperCase() || '';
    items.push({
      ...
      actual: matchedPlan.periodType || 'unknown',
      details: actualPeriod === plannedPeriod
        ? `Both ${config.planStructure.periodType}`
        : `Planned: ${config.planStructure.periodType}, Actual: ${matchedPlan.periodType || 'not set'}`,
    });

    if (matchedPlan.status) {
      items.push({
        ...
        status: 'match',
        planned: null,
        actual: matchedPlan.status,
        details: `Plan status: ${matchedPlan.status}`,
      });
    }
  } else {
    items.push({
  ...
  if (components.length > 0) {
    items.push({
      ...
      name: 'Payout components',
      category: 'plan-structure',
      priority: 'required',
      status: 'partial',
      planned: components.join(', '),
      actual: 'not available in extraction',
      details: 'Component data not available in extraction — verify manually in CIQ UI',
    });
  }

  // Extra plans in extraction not matching config
  const unmatchedPlans = selectedPlans.filter(p => !fuzzyMatch(config.planStructure.planName, p.name));
  if (unmatchedPlans.length > 0) {
    items.push({
      id: 'plan-structure.extra-plans',
      name: 'Extra plans in CIQ',
      category: 'plan-structure',
      priority: 'informational',
      status: 'extra-in-ciq',
      planned: null,
      actual: unmatchedPlans.map(p => p.name).join(', '),
      details: `${unmatchedPlans.length} additional plans in extraction`,
    });
  }

  // ── Data Worksheets ─────────────────────────────────────

  const liveWsNames = allWorksheets.map(ws => ws.name);
  const plannedWs = config.dataWorksheets;

  for (const pw of plannedWs) {
    const match = allWorksheets.find(ws => fuzzyMatch(pw.name, ws.name));
    if (!match) {
      items.push({
        ...
        actual: null,
        details: `Worksheet "${pw.name}" not found. Closest: ${findBestMatch(pw.name, liveWsNames) || 'none'}`,
      });
      continue;
    }
    ...
    if (match.columns && match.columns.length > 0) {
      const liveColNames = match.columns.map(c => c.name);
      ...
      items.push({
        ...
        details: missingCols.length === 0
          ? extraCols.length === 0 ? 'All columns match' : `Extra columns: ${extraCols.join(', ')}`
          : `Missing: ${missingCols.join(', ')}${extraCols.length ? '; Extra: ' + extraCols.join(', ') : ''}`,
      });

      // Column type check
      const typeMismatches: string[] = [];
      for (const pc of pw.columns) {
        const liveCol = match.columns.find(lc => fuzzyMatch(pc.name, lc.name));
      ...
      if (pw.columns.length > 0) {
        items.push({
          ...
          actual: match.columns.map(c => `${c.name}:${c.type}`).join(', '),
      ...
      items.push({
        ...
        details: `Worksheet "${match.name}" found but column details not available`,
      });
    }
  }
  ...
  items.push({
    ...
    category: 'data-worksheets',
    priority: 'recommended',
    status: allWorksheets.length > 0 ? 'match' : 'missing-in-ciq',
    planned: `${plannedWs.length} worksheets expected`,
    actual: `${allWorksheets.length} worksheets found across ${extraction.workbooks.length} workbooks`,
    details: `${allWorksheets.length} worksheets in ${extraction.workbooks.length} workbooks`,
  });

  // Extra worksheets
  const unmatchedLiveWs = allWorksheets.filter(ws =>
  ...
  if (unmatchedLiveWs.length > 0) {
    items.push({
      ...
      details: `${unmatchedLiveWs.length} worksheets in CIQ not in plan`,
    });
  }
  ...
  const assumptionCols = extraction.employeeAssumptions?.columns || [];
  if (assumptionCols.length > 0) {
    for (const pc of plannedCols) {
      const match = findBestMatch(pc.name, assumptionCols);
      items.push({
        ...
        details: match
          ? `Found as "${match}"`
          : `Column "${pc.name}" not found. Available: ${assumptionCols.slice(0, 10).join(', ')}`,
      });
    }

    // Extra columns
    const extraCols = assumptionCols.filter(c => !findBestMatch(c, plannedCols.map(pc => pc.name)));
    ...
    items.push({
      ...
      actual: null,
      details: 'No employee assumption data in extraction',
    });
  }
  ...
  for (const pa of plannedAttrs) {
    const match = extraction.attributeWorksheets.find(a => fuzzyMatch(pa.name, a.name));
    if (!match) {
      items.push({
        ...
        actual: null,
        details: `Attribute worksheet "${pa.name}" not found`,
      });
      continue;
    }
    ...
    if (match.columns && match.columns.length > 0) {
      ...
      items.push({
        ...
        actual: match.name,
        details: 'Attribute worksheet found, column detail not available',
      });
    }
  }

  // Extra attribute worksheets
  const unmatchedAttrs = extraction.attributeWorksheets.filter(a =>
  ...
  if (plannedAttrs.length > 0) {
    items.push({
      ...
      category: 'attribute-worksheets',
      priority: 'recommended',
      status: extraction.attributeWorksheets.length > 0 ? 'match' : 'missing-in-ciq',
      planned: `${plannedAttrs.length} attribute worksheets`,
      actual: `${extraction.attributeWorksheets.length} found`,
      details: `${extraction.attributeWorksheets.length} attribute worksheets in extraction`,
    });
  }
  ...
  if (formulas.length > 0) {
    const derivedCount = allWorksheets.reduce((count, ws) => {
      return count + ws.columns.filter(c => c.isDerived === true).length;
    }, 0);
  ...
  return {
    ...
    liveSummary: {
      ...
      workbookCount: extraction.workbooks.length,
      worksheetCount: allWorksheets.length,
      attributeWorksheetCount: extraction.attributeWorksheets.length,
      employeeCount: 0,
    },
    counts,
  };
